...
class SecondGameWindow(QMainWindow):
    def __init__(self, user_id):
        super().__init__()
        uic.loadUi('uifiles/second_game.ui', self)
    ...
    def ok_button(self):
        if self.isStartedTimer:
            ...
            if self.lineEdit.text() != '' and self.lineEdit.text().isdigit():
                num = int(self.lineEdit_3.text()[1:])
                result = int(self.lineEdit.text())
                if math.sqrt(num) == result:
                    self.process_correct_answer()
                elif math.sqrt(num) != result:
    ...
    def show_end_dialog(self):
        ...
        if int(mistakes.split(': ')[1]) == 0:
            ...
            con = sqlite3.connect('datebase.db')
            cur = con.cursor()
            get_score = [i[0] for i in cur.execute(f'SELECT {nums[str(self.counterTask)]} FROM sq_table WHERE user_id = {self.user_id}')]
            con.commit()
            ...
            if milisec2 < milisec or milisec == 0:
                cur.execute(f'UPDATE sq_table SET {nums[str(self.counterTask)]} = "{target_score_t}" WHERE user_id = {self.user_id}')
                con.commit()
            cur.close()
    ...
    def gen_numbers(self):
        # num0 = 9
        num0 = random.choice(list(self.squads.keys()))
        self.lineEdit.setText('')
        self.lineEdit_3.setText(f'√{num0}')

    def start_timer(self):
        if self.config_data["volume_button_status"]:
            self.button_sound.play()
        self.stop_timer()
        count, ok_pressed = QInputDialog.getItem(self, "Выберите количество примеров", "Количество примеров", ('5', '10', '20', '25', '40'), 1, False)
        if ok_pressed:
